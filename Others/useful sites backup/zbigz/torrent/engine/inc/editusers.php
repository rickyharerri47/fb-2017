...
HTML;
	...
	while ( $row = $db->get_row() ) {
		...
		if( $row[news_num] == 0 ) {
			...
			$news_link = "<a class=\"status-info tip\" href=\"{$config['http_home_url']}index.php?subaction=allnews&user=" . urlencode( $row['name'] ) . "\" target=\"_blank\" data-original-title=\"{$lang['rss_maxnews']}\">" . $row[news_num] . "</a>";
		}
		if( $row[comm_num] == 0 ) {
