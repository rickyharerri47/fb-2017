...
if ( strtolower($config['charset']) != $distr_charset ) {
	msgbox("info", "Information", "L'encodage de votre site ne correspond pas à la distribution de codage. Votre site utilise le codage: <b>{$config['charset']}</b>,et la distribution sur lequel vous essayez de mettre à niveau contient codage: <b>{$distr_charset}</b>. Vous devez télécharger le package dans l'encodage approprié de continuer la mise à niveau.");
	die();
}
