...
class AuthViaOdnoklassniki {  
    ...
    function get_user( $social_config ) {
		...
		if(isset($_GET['email'])) $ext_uri="&email=".$_GET['email']."&state=".$_SESSION['state']; else $ext_uri = "&state=".$_SESSION['state'];
		...
		$params = array(
			...
			'grant_type' => 'authorization_code',
			'code' => $_GET['code'],
			'redirect_uri'  => $config['http_home_url'] . "index.php?do=auth-social&provider=od".$ext_uri
		);
