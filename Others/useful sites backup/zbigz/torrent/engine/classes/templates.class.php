...
class dle_template {
	...
	function load_template($tpl_name) {
		...
		if( stripos ( $tpl_name, ".php" ) !== false ) die( "Template non autorisé: " . str_replace(ROOT_DIR, '', $this->dir)."/".$tpl_name );	

		if( $tpl_name == '' || !file_exists( $this->dir . "/" . $tpl_name ) ) {
			die( "Fichier manquant: " . str_replace(ROOT_DIR, '', $this->dir)."/".$tpl_name );
			return false;
		}
	...
	function load_file( $matches=array() ) {
		...
		if ($this->include_mode == "php") {
			...
			if ($type != "php") return "Seuls les fichiers avec l'extension autorisés: .tpl or .php";
	...
	function sub_load_template( $tpl_name ) {
		...
		if ($type != "tpl") {
			...
			return "Template non autorisé: ". $tpl_name;
		...
		if( $tpl_name == '' || !file_exists( $templatefile ) ) {
			...
			return "Fichier manquant: " . $templatefile ;
			return false;

		}

		if( stripos ( $templatefile, ".php" ) !== false ) return "Template non autorisé: ". $tpl_name;
