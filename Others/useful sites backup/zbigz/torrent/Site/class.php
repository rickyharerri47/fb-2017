...
class getinfo {
	...
	function notice(){
		...
		printf($this->lang['status'],$maxsize,count($this->jobs));
	}
...
class stream_get extends getinfo {
	...
	function get($url){
		...
		if(!$link){
			...
			if($size_name[0]>1024*5) $link=$url;
			else {
		...
		$linkdown = 'http://'.$sv_name.$hosting.'/'.$job['hash'].'/'.$filename;
		}else{
		$linkdown = $link;
		}
		
		##########Begin short link ############
		if(empty($this->zlink)==false && empty($this->link1_zip)==false && empty($link)==false ){ 
			$datalink = $this->curl($this->link1_zip.$linkdown,'','',0);
...
echo " <br>"; 
         echo "<a title='Your Link URL' href='$lik' style='TEXT-DECORATION: none' target='$tiam'><img src='http://i.imgur.com/gd73vH1.png' width='200' height='120' alt='Download Here' title='Download Here'></a><br><br><b><a title='click here to download' href='$lik' style='TEXT-DECORATION: none' target='$tiam'><font color=#ff0000 size='2px'><b>File Name : </b></font><font color='#000000'>".$filename." </font><font color=#ff0000 size='2px'> <b>  File Size : </b></font> <font color='#000000'> $msize </font></a></b><br>";
         echo " <br>"; 
		return $dlhtml;
	}
